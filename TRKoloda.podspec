#
# Be sure to run `pod lib lint TRKoloda.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'TRKoloda'
  s.version          = '0.1.0'
  s.summary          = 'TRKolodaView is a class designed to simplify the implementation of Tinder like cards on iOS. '

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
                         TRKolodaView is a class designed to simplify the implementation of Tinder like cards on iOS
                       DESC

  s.homepage         = 'https://github.com/TrumpRothschild/TRKoloda'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'TrumpRothschild' => '184889355@qq.com' }
  s.source           = { :git => 'git@github.com:TrumpRothschild/TRKoloda.git', :tag => s.version.to_s }
  s.social_media_url = 'https://twitter.com/DonaldJoseph143'

  #s.ios.deployment_target = '10.0'
  s.platform     = :ios, '8.0'
  s.source_files = 'TRKoloda/Classes/**/*'
  
  # s.resource_bundles = {
  #   'TRKoloda' => ['TRKoloda/Assets/*.png']
  # }
  
  # s.public_header_files = 'Pod/Classes/**/*.h'
  s.frameworks = 'UIKit'
  s.dependency 'pop', '~> 1.0'
  # s.dependency 'AFNetworking', '~> 2.3'
end
